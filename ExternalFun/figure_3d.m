...
function hFig = CreateFigure(FigureId) %#ok<DEFNU>
    ...
    % === CREATE AXES ===
    global hdl % cuiwei
    hdl.hpanel = uipanel('Parent', hFig,'Position', hdl.SurfPos, 'BackgroundColor',[1 1 1], 'Tag', 'SurfPan', 'BorderWidth', 0.5);
    hAxes = axes('Parent',        hdl.hpanel, ...
                 'Units',         'normalized', ...
                 'Position',      [.05 .05 .9 .9], ...
                 'Tag',           'Axes3D', ...
                 'Visible',       'off', ...
...
function ResizeCallback(hFig, ev)
    ...
    hAxes     = findobj(hFig, '-depth', 2, 'Tag', 'Axes3D');
    if isempty(hAxes)
        return
    end
    hAxes = hAxes(1);
...
function FigureClickCallback(hFig, varargin)
    % cuiwei
    hPan = findobj(hFig, '-depth', 1, 'Tag', 'SurfPan');
    Axes1Pos(1) = hFig.Position(3)*hPan.Position(1);
    Axes1Pos(2) = hFig.Position(4)*hPan.Position(2);
    Axes1Pos(3) = hPan.Position(3)*hFig.Position(3);
    Axes1Pos(4) = hPan.Position(4)*hFig.Position(4);
    Position = get(hFig, 'CurrentPoint');
    if(Position(2)>Axes1Pos(2)&&Position(2)<(Axes1Pos(2)+Axes1Pos(4))&&...
       Position(1)>Axes1Pos(1)&&Position(1)<(Axes1Pos(1)+Axes1Pos(3)))
...
function FigureMouseMoveCallback(hFig, varargin)  
    ...
    % Get figure size (cuiwei)
    figPos = get(hFig, 'Position');
%     hPan = findobj(hFig, '-depth', 1, 'Tag', 'SurfPan');
%     pannelPos = get(hPan, 'Position');
%     pannelPos(1) = figPos(1)+figPos(3)*pannelPos(1);pannelPos(2) = figPos(2)+figPos(4)*pannelPos(2);
%     pannelPos(3) = figPos(3)*pannelPos(3);pannelPos(4) = figPos(4)*pannelPos(4);
%     figPos = pannelPos;
...
function ApplyViewToAllFigures(hSrcFig, isView, isSurfProp)
    % Get Axes handle
    hSrcAxes = findobj(hSrcFig, '-depth', 2, 'Tag', 'Axes3D');
    ...
    for i = 1:length(hAllFig)
        % Get Axes handle
        hDestFig = hAllFig(i);
        hDestAxes = findobj(hDestFig, '-depth', 2, 'Tag', 'Axes3D');
...
%             jItem3.setSelected(MriOptions.InterpDownsample == 3);
    ...
    jMenuFigure = gui_component('Menu', jPopup, [], 'Figure', IconLoader.ICON_LAYOUT_SHOWALL);
        ...
        gui_component('MenuItem', jMenuFigure, [], 'Change background color', IconLoader.ICON_COLOR_SELECTION, [], @(h,ev)figure_3d('SetBackgroundColor', hFig));  % cuiwei
        jMenuFigure.addSeparator(); 
...
function [hElectrodeGrid, ChanLoc] = PlotSensors3D(iDS, iFig, Channel, ChanLoc, TopoType) %#ok<DEFNU>
    ...
    % Get axes
    hFig = GlobalData.DataSet(iDS).Figure(iFig).hFigure;
    hAxes = findobj(hFig, '-depth', 2, 'tag', 'Axes3D');  % cuiwei
    % Get font size
...
function SetBackgroundColor(hFig, newColor) %#ok<*DEFNU>
    ...
    hdl.hpanel.BackgroundColor = newColor;
    hdl.Pannel_MEG.BackgroundColor = newColor;
