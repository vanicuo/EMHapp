...
class temporal_cluster:
    ...
    def similarity_between_segments_templates(self, data, templates, chunk=5000):
        ...
        # 转换为和输入data相同维度
        sim_raw_format = torch.zeros(data.shape[0], data.shape[1], data.shape[2], templates.shape[0]).to(self.device)
        ...
        self.del_var()
        return sim_raw_format, corr_raw_format, dist_raw_format
    ...
    def cal_similarity_matrix_with_pearson_euler_chunk(self, data_x, data_y, chunk=1000):
        ...
        with torch.no_grad():
            ...
            corr = torch.zeros(data_x.shape[0], data_y.shape[0]).to(self.device).double()
    ...
    def cal_similarity_matrix_with_pearson_euler(self, data_x, data_y):
        ...
        with torch.no_grad():
            ...
            # 转换为矩阵格式
            corr = corr_temp.nan_to_num_(0).reshape(data_x.shape[0], data_y.shape[0]).double()
            dist = dist_temp.nan_to_num_(0).reshape(data_x.shape[0], data_y.shape[0]).double()
            sim = sim_temp.nan_to_num_(0).reshape(data_x.shape[0], data_y.shape[0]).double()
    ...
    def similarity_matrix_with_pearson_euler(self, data, chunk=1000):
        ...
        index_with_max_sim = torch.stack(sim, dim=-1).argmax(dim=-1).unsqueeze(-1)
        sim_shift = torch.stack(sim, dim=-1)
        sim = sim_shift.take_along_dim(index_with_max_sim, dim=-1).squeeze()
        corr_shift = torch.stack(corr, dim=-1)
        corr = corr_shift.take_along_dim(index_with_max_sim, dim=-1).squeeze()
        dist_shift = torch.stack(dist, dim=-1)
    ...
    def template_matching(self, ied_amplitude_peaks=None, large_peak_amplitude_threshold=0.80,
        ...
        sim, corr, dist = self.similarity_between_segments_templates(data_segment_all, clusters_center_data)

        # 获取大于阈值的ied event
        ied_index = ((sim.amax(dim=-1) > sim_threshold) &
                     (corr.amax(dim=-1).abs() > corr_threshold) &
                     (dist.amax(dim=-1) > dist_threshold)).sum(dim=-1) >= channel_threshold

        return ied_index, clusters_data, clusters_center_data
    ...
    def plot_clusters(clusters_data=None, clusters_center_data=None, figsize=(10, 25.6), dpi=100, figure_col=10):
        ...
        if clusters_data is not None:
            ...
            fig = plt.figure(num=233, figsize=(figsize[0], round(len(clusters_data) / figure_col + 0.5) * 1),
                             clear=True, dpi=dpi)
            # 计算每个类别位置
            ...
            for i in cls_range:
                ...
                ax.text(ax.get_xlim()[0] + 10, ax.get_ylim()[1] - 0.01,
                        'Cls: ' + str(i) + ';  ' + 'Num: ' + str(clusters_data[i].shape[0]))
            # 画类别之间的虚线
