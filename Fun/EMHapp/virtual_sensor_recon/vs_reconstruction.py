...
class vs_recon:
    ...
    def cal_noise_covariance(self, raw_data_ied=None, raw_data_hfo=None, re_lambda=0.05, noise_win=10,
        ...
        with torch.no_grad():
            ...
            if raw_data_hfo is not None:
                # 删除IED片段
                sample_index = torch.arange(0, int(noise_win * self.data_info['sfreq']))
                if ied_peak_sample is not None:
                    temp = int(0.2 * self.data_info['sfreq'])
                    ied_sample_index = (torch.tensor(ied_peak_sample) +
                                        torch.arange(-temp, temp).reshape(-1, 1)).reshape(-1)
                    ied_sample_index = ied_sample_index[(ied_sample_index >= sample_index[0]) &
                                                        (ied_sample_index < sample_index[-1])]
                    sample_index = sample_index[torch.tensor([x for x in sample_index if x not in ied_sample_index])]
                noise_data_hfo_temp = raw_data_hfo[:, sample_index].to(self.device)
                noise_data_hfo_temp = noise_data_hfo_temp[:, ~self.bad_segment[sample_index]]
    ...
    def sam_costfun_peak2peak(self, data, leadfields, inv_covariance,
        ...
        data_interest = segment_data_conv(data_interest.unsqueeze(1).float())
        ...
        data_interest = segment_data_conv(data_interest.reshape(grids_num * fine_ori_samples,
                                                                -1).unsqueeze(1).unsqueeze(1).float())
...
if __name__ == "__main__":
    ...
    matFile = '/data2/cuiwei/HFO_Detection_Results/MaxOri/CHY/caohaiyan/@rawcaohaiyan3_tsss/SaveVirtualSensorResults.mat'
