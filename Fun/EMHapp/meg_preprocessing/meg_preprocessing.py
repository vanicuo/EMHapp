...
class preprocess:
    ...
    def cal_ecg_eog_channels(self, raw, check_ecg_and_eog_method=1, out_dir=None):
        ...
        if len(ecg_ref_ch) > 0 and len(eog_ref_ch) > 0:
            ...
            eog_data = mne.filter.filter_data(raw.get_data(picks=eog_ref_ch[0]), raw.info['sfreq'],
                                              l_freq=1., h_freq=35., verbose='error',
            ...
            if check_ecg_and_eog_method == 1:
                ...
                (ecg_ref_ch, eog_ref_ch) = (ecg_ref_ch[0], eog_ref_ch[0]) \
                    if len(np.where(ecgHist > 0.75 * ecgHist.max())[0]) <= \
                       len(np.where(eogHist > 0.75 * eogHist.max())[0]) else (eog_ref_ch[0], ecg_ref_ch[0])
            # 确定是否能找到EOG事件
            ...
            if ecg_events.shape[0] > 0:
                ecg_ref_ch = ref_ch
                eog_ref_ch = None
                self.save_ecg_eog_sample(out_dir=out_dir, ecg_data=raw.get_data(picks=ecg_ref_ch)[0, 4000:6000])
            elif eog_events.shape[0] > 0:
                eog_ref_ch = ref_ch
                ecg_ref_ch = None
                self.save_ecg_eog_sample(out_dir=out_dir, eog_data=raw.get_data(picks=eog_ref_ch)[0, 4000:6000])
            else:
                eog_ref_ch = None
                ecg_ref_ch = None
        else:
    ...
    @staticmethod
    def save_ecg_eog_components(ica=None, ecg_scores=None, eog_scores=None, out_dir=None):
        """
        Description:
            保存ECG/EOG片段图片。
    ...
    def save_fif(raw, out_dir=None):
        ...
        Input:
            ...
            :param out_dir: path
                fif文件存储位置
        """
        raw.save(os.path.join(out_dir, os.path.split(raw.filenames[0])[-1].split('.')[-2] + '_tsss.fif'),
                 overwrite=True)
    ...
    def run_process(self, fif_file, out_dir=None,
        ...
        try:
            ...
            fif_name = os.path.split(raw.filenames[0])[-1].replace('_tsss', '')
            # 新建log文件
            if self.is_log:
                if not os.path.exists(os.path.join(self.log_out_dir, re.compile(r'_EP_\d*\.fif').split(fif_name)[0])):
                    os.makedirs(os.path.join(self.log_out_dir, re.compile(r'_EP_\d*\.fif').split(fif_name)[0]),
                                exist_ok=True)
                log_out_dir = os.path.join(self.log_out_dir, re.compile(r'_EP_\d*\.fif').split(fif_name)[0],
                                           fif_name.replace('.fif', '_logs.txt'))
            # 进行tsss处理
            if do_tsss:
                if self.is_log:
                ...
                out_dir_tsss = os.path.join(out_dir, 'MEG_TSSS', re.compile(r'_EP_\d*\.fif').split(fif_name)[0])
                if not os.path.exists(out_dir_tsss):
                    os.makedirs(out_dir_tsss, exist_ok=True)
                raw = self.cal_maxwell_filter(raw, tsss_lowpass=tsss_auto_bad_lowpass, thre=tsss_auto_bad_thre,
                                              ...
                                              manual_bad=tsss_manual_bad, out_dir=out_dir_tsss)
                self.save_fif(raw, out_dir_tsss)
            # 进行ICA处理
            if do_ica:
                if self.is_log:
                    self.set_log_file(fname=log_out_dir,
                                      output_format='[%(levelname)s][ICA][' + fif_name.replace('.fif', '') +
                                                    ']: <line:%(lineno)d> %(message)s', overwrite=False)
                out_dir_ica = os.path.join(out_dir, 'MEG_ICA', re.compile(r'_EP_\d*\.fif').split(fif_name)[0])
                if not os.path.exists(out_dir_ica):
                    os.makedirs(out_dir_ica, exist_ok=True)
                raw = self.cal_ecg_eog_artifact_removal(raw, out_dir=out_dir_ica)
                if do_resample and resample_freq is not None:
                    raw = self.cal_resample(raw, resample_freq=resample_freq)
                self.save_fif(raw, out_dir_ica)
                ...
                if self.is_log:
                    self.set_log_file(fname=log_out_dir,
                                      output_format='[%(levelname)s][Jump][' + fif_name.replace('.fif', '') +
                                                    ']: <line:%(lineno)d> %(message)s', overwrite=False)
                out_dir_segment = os.path.join(out_dir, 'MEG_BAD', re.compile(r'_EP_\d*\.fif').split(fif_name)[0])
            ...
            if do_filter:
                ...
                if 'IED' in filter_band:
                    out_dir_ied = os.path.join(out_dir, 'MEG_IED', re.compile(r'_EP_\d*\.fif').split(fif_name)[0])
                ...
                if 'HFO' in filter_band:
                    out_dir_hfo = os.path.join(out_dir, 'MEG_HFO', re.compile(r'_EP_\d*\.fif').split(fif_name)[0])
            ...
            print('Preprocessing finished: ', fif_name.split('.')[-2])
...
if __name__ == "__main__":
    ...
    Fif_files = ['/sanbo_dataset/4k_Project/Data_Full/MEG_RAW/MEG2402/MEG2402_EP_9.fif']
    BadChannel_Dir = '/sanbo_dataset/4k_Project/Data_Full/MEG_BadChannel/txt_file/IED'

    MEG = preprocess(fif_files=Fif_files, out_dir='/data2/cuiwei/HFO_IED_Ictal/TEST', bad_channel_dir=BadChannel_Dir,
                        device=0, n_jobs=10)
    MEG.run_multi_fifs(do_tsss=True, do_ica=True, do_resample=False,
