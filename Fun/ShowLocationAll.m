function ShowLocationAll(FigureName, ShowResult, SourceGrid, MRI, Surface)
    global hdl
    % initial Param
    SurfAlpha = 0.75;DataAlpha = 0.55;DataThres = 0.8;SurfSmooth = 0;ResAlpha = 0.6;
    % Add Path
    ...
    hdl.MRI = strrep(MRI, [fileparts(fileparts(Surface)), '/'], '');hdl.Surface = strrep(Surface, [fileparts(fileparts(Surface)), '/'], '');
    hdl.MRI_Mat = load(fullfile(Protocol.SUBJECTS, hdl.MRI));
    hdl.ProtocolPath = fileparts(fileparts(fileparts(Surface)));
    hdl.FigureName = FigureName;hdl.SourceGrid = SourceGrid;
    hdl.SeegChannelMat = [];
    % Add surf and Show Results
    ShowSurface(DataThres, SurfSmooth, SurfAlpha, DataAlpha);
...
function CallFunSizeChange(~,~,~)
    ...
    if(~hdl.ShowSurface)
        ...
        hColorbar = findobj(hdl.Fig, '-depth', 2, 'Tag', 'Colorbar');
        set(hColorbar,'FontSize',hdl.LabelSize/2);
    end
end
...
%         text(hdl.axes,Pos(1,:),Pos(2,:),Pos(3,:)+0.001,Name,'color','w','FontSize',hdl.LabelSize/4,'FontWeight','bold','Tag','SEEGTag');
        ...
        TempDir = ['Channel_',num2str(round(rand(1)*100000)),'.mat']; 
        ...
        if(~hdl.ShowSurface&&~isempty(hdl.SeegChannelMat)&&~isempty(findobj(hdl.Fig, '-depth', 3, 'Tag', 'ElectrodeGrid')))
...
function Callback_HFOAlpha(~,~,~)
    ...
    if(hdl.ShowSurface)
        TessInfo = getappdata(hdl.Fig, 'Surface');iSurface = find(cellfun(@(x)strcmp(x, 'Cortex'),  {TessInfo.Name}));
        TessInfo(iSurface).DataAlpha = Transp;setappdata(hdl.Fig, 'Surface', TessInfo);
        figure_3d('UpdateSurfaceColor', hdl.Fig, iSurface);
    end
...
function Callback_ShowAmp(~,~,~)
    ...
    ShowSurface(min(max(hdl.Slider_ResultsThreshold.Value, 0.01), 0.99), min(max(hdl.Slider_SurfaceSmooth.Value, 0.01), 0.99), ...
                min(max(hdl.Slider_SurfaceAlpha.Value, 0.01), 0.99), min(max(hdl.Slider_HFOAlpha.Value, 0.01), 0.99));
end
...
function CallFun_ResultsThreshold(~,~,~)
    ...
    if(~isempty(hdl.Loc)&&((peak2peak(double(hdl.Map))>=1 && ~hdl.ShowAmp.Value) || hdl.ShowAmp.Value))       
        ...
        if(~hdl.ShowSurface)
            ...
            TessInfo = getappdata(hdl.Fig, 'Surface');iSurface = find(cellfun(@(x)strcmp(x, 'Cortex'),  {TessInfo.Name}));
            hColorbar = findobj(hdl.Fig, '-depth', 2, 'Tag', 'Colorbar');
            set(hColorbar,'FontSize',hdl.LabelSize / 2,'FontWeight','bold');
            set(hColorbar,'YTick',0:256/5:256);
            set(hColorbar,'YTickLabels',cellfun(@(x)num2str(round(x*100)/100),num2cell(double(Thres):(double(max(hdl.Map)) - double(Thres)) / (length(hColorbar.YTick) - 1):double(max(hdl.Map))),'UniformOutput',false));
            % Set Threshold
            ...
            TessInfo(iSurface).Data = hdl.ReaultsSurf_Show;TessInfo(iSurface).Data(hdl.ReaultsSurf_Show<double(Thres)) = 0;
            % Update current surface
            setappdata(hdl.Fig, 'Surface', TessInfo);
            FigureId = getappdata(hdl.Fig, 'FigureId');
            figure_3d('UpdateSurfaceColor', hdl.Fig, iSurface);
        end
        % Updata MRI Threshold
        if(isfield(hdl, 'hFig_MRI')&&isvalid(hdl.hFig_MRI))
...
function Callback_ShowMRI(~,~,~)
    ...
    Thres = round(peak2peak(hdl.Map) * min(max(hdl.Slider_ResultsThreshold.Value, 0.01), 0.99) * 100) / 100;
    if(Show_VS==1)
        Map = double(hdl.Map);Loc = hdl.Loc';Loc(:, Map<double(Thres)) = [];Map(Map<double(Thres)) = [];
        Diameter = 5;
        % Get Mri and modify GlobalData
        TempDir = fileparts(file_fullpath(hdl.ProtocolFile.ProtocolSubjects(1).Subject.Anatomy(hdl.ProtocolFile.ProtocolSubjects(1).Subject.iAnatomy).FileName));
        hdl.MRI_TempDir = fullfile(TempDir, ['subjectimage_',num2str(round(rand(1)*100000)),'.mat']);    
        temp = length(GlobalData.DataBase.ProtocolSubjects(GlobalData.DataBase.iProtocol).Subject(1).Anatomy);
        GlobalData.DataBase.ProtocolSubjects(GlobalData.DataBase.iProtocol).Subject(1).Anatomy(temp+1).FileName = file_short(hdl.MRI_TempDir);
        % Make the overlay file
        sMRI = load(fullfile(hdl.ProtocolPath, 'anat', hdl.MRI));sMRI.Cube = zeros(size(sMRI.Cube));
        for i=1:length(Map)
            LocTemp = round(cs_convert(sMRI, 'scs', 'voxel',Loc(:,i)));
            [X,Y,Z] = meshgrid(LocTemp(1)-Diameter/2:1:LocTemp(1)+Diameter/2,LocTemp(2)-Diameter/2:1:LocTemp(2)+Diameter/2,LocTemp(3)-Diameter/2:1:LocTemp(3)+Diameter/2);
            Temp = round(cat(2,X(:),Y(:),Z(:)));
            Temp = Temp(sum((Temp-LocTemp).^2,2)<=(Diameter/2)^2,:);
            sMRI.Cube(sub2ind(size(sMRI.Cube),Temp(:,1),Temp(:,2),Temp(:,3))) = double(Map(i));
        end
        save(hdl.MRI_TempDir,'-struct','sMRI');
        % Overlay
        [hdl.hFig_MRI, ~, ~] = view_mri(hdl.MRI, hdl.MRI_TempDir);hdl.hFig_MRI.Name = hdl.FigureName;
        iSurface = getappdata(hdl.hFig_MRI, 'iSurface');TessInfo = getappdata(hdl.hFig_MRI, 'Surface');
        TessInfo(iSurface).DataThreshold = 0;
        TessInfo(iSurface).DataMinMax = [double(Thres), max(double(hdl.Map))];
        TessInfo(iSurface).DataLimitValue = [double(Thres), max(double(hdl.Map))];
        setappdata(hdl.hFig_MRI, 'Surface', TessInfo);
        ...
        Map = double(hdl.Map);Loc = hdl.Loc;
        % Get Mri and modify GlobalData
        TempDir = fullfile(hdl.ProtocolFile.ProtocolSubjects(1).Subject.Name, 'Temp',['results',num2str(round(rand(1)*100000)),'.mat']); 
        % Modify GlobalData
        if(~strcmp(GlobalData.DataBase.ProtocolStudies(GlobalData.DataBase.iProtocol).Study(end).Name, 'Temp'))
        ...
        TessInfo(iSurface).Data = Map;TessInfo(iSurface).Data(Map<double(Thres)) = 0;
...
function ShowSurface(Thre, Smooth, Alpha, DataAlpha)
    global hdl GlobalData
    % Add surf and Show Results
    if(hdl.ShowSurface)
        % Get Surface
        Protocols = load(bst_get('BrainstormDbFile'));
        Surf = load(fullfile(hdl.ProtocolPath, 'anat',hdl.Surface));SurfPos = Surf.Vertices;
        % Interp Grid to Surface
        cfg = [];
        cfg.interpmethod = ft_getopt(cfg, 'interpmethod', 'nearest');
        cfg.sphereradius = ft_getopt(cfg, 'sphereradius', 0.1);
        cfg.power        = ft_getopt(cfg, 'power',        1);
        [interpmat, ~] = interp_gridded(hdl.SourceGrid.transform, zeros(hdl.SourceGrid.dim), SurfPos, 'projmethod', cfg.interpmethod, 'sphereradius', cfg.sphereradius, 'inside', hdl.SourceGrid.inside, 'power', cfg.power);
        hdl.Reaults_Surf = interpmat * double(hdl.Map);hdl.ReaultsSurf_Show = interpmat * double(hdl.Map);
        % Modify GlobalData
        TempDir = fullfile(hdl.ProtocolFile.ProtocolSubjects(1).Subject.Name, 'Temp',['results',num2str(round(rand(1)*100000)),'.mat']); 
        % Modify GlobalData
        if(~strcmp(GlobalData.DataBase.ProtocolStudies(GlobalData.DataBase.iProtocol).Study(end).Name, 'Temp'))
        ...
        GlobalData.DataBase.ProtocolStudies(GlobalData.DataBase.iProtocol).Study(end).Result(length(GlobalData.DataBase.ProtocolStudies(GlobalData.DataBase.iProtocol).Study(end).Data) + 1).FileName = TempDir;
        hdl.Reaults_Surf_Dir = fullfile(hdl.ProtocolPath, 'data', TempDir);  
        % Make Results File
        results = db_template('resultsmat');
        results.ImageGridAmp = hdl.ReaultsSurf_Show;results.Time = 0;results.SurfaceFile = hdl.Surface;
        save(fullfile(hdl.ProtocolPath, 'data', TempDir),'-struct','results');
        % Show
        hdl.SurfPos = [0.00 0.000 0.75 1.00];
        if(~isfield(hdl, 'Fig') || ~isvalid(hdl.Fig))
            [hdl.Fig, ~, ~] = view_surface_data(hdl.Surface, TempDir, 'MEG', 'NewFigure', 0);set(hdl.Fig,'Name',hdl.FigureName,'NumberTitle', 'off'); 
        end
        % Update current surface (apply Threshold and colorbar)
        TessInfo = getappdata(hdl.Fig, 'Surface');iSurface = find(cellfun(@(x)strcmp(x, 'Cortex'),  {TessInfo.Name}));
        Thres = round(peak2peak(hdl.Map) * min(max(Thre, 0.01), 0.99) * 100) / 100;
        TessInfo(iSurface).DataThreshold = 0;
        TessInfo(iSurface).DataMinMax = [double(Thres), max(double(hdl.Map))];
        TessInfo(iSurface).DataLimitValue = [double(Thres), max(double(hdl.Map))];
        TessInfo(iSurface).Data = hdl.Reaults_Surf;TessInfo(iSurface).Data(hdl.Reaults_Surf<double(Thres)) = 0;TessInfo(iSurface).DataAlpha = round(DataAlpha*100)/100;
        TessInfo(iSurface).SurfSmoothValue=round(Smooth*100)/100;setappdata(hdl.Fig, 'Surface', TessInfo);figure_3d('UpdateSurfaceAlpha', hdl.Fig, iSurface);
        TessInfo(iSurface).SurfAlpha = Alpha;
        setappdata(hdl.Fig, 'Surface', TessInfo);FigureId = getappdata(hdl.Fig, 'FigureId');
        figure_3d('UpdateSurfaceColor', hdl.Fig, iSurface);figure_3d('UpdateSurfaceAlpha', hdl.Fig, iSurface); 
