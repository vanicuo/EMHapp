function appTemp = StartFcn(appTemp)
    % bst Init
    brainstorm setpath
    ...
    ft_defaults();
    addpath(fileparts(which('NetGroupProcess')));
    % Set Database folder
    global GlobalData;
    try
        % Select new database folder
        bst_set('BrainstormDbDir', appTemp.BrainstormDbDir);
        ...
        % Import new database
        db_import(appTemp.BrainstormDbDir);
    catch
        bst_set('BrainstormDbDir', appTemp.BrainstormDbDir);
        % Get the current protocol: if not empty, there are existing protocols to unload
        iiProtocol = bst_get('iProtocol');
